...
export default function ProjectDetailModal({ setDetailOpen, detailId }: Props) {
  ...
  const getDetailData = async () => {
    try {
      const res = await axios.get(`/project/detail/${detailId}?memId=${typeof window !== "undefined" ? sessionStorage.getItem("memId") : ""}`);
  ...
  const onJoinClick = async () => {
    try {
      const res = await axios.post(`/project/participation?memId=${typeof window !== "undefined" ? sessionStorage.getItem("memId") : ""}`, {
        PRO_ID: detailId,
      });
      if (res.status === 200) {
